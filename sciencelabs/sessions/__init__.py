...
class SessionView(FlaskView):
    ...
    @route('/checkin/<int:session_id>/<session_hash>/<username>', methods=['get'])
    # @route('/no-cas/checkin/<int:session_id>/<session_hash>/<card_id>', methods=['get', 'post'])
    def student_sign_in(self, session_id, session_hash, username):
    ...
    def _logout_caleb(self, service_path):
        ...
        resp.set_cookie('MOD_AUTH_CAS_S', '', expires=1)
        resp.set_cookie('MOD_AUTH_CAS', '', expires=1)
        return resp
