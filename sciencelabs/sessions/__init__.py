...
class SessionView(FlaskView):
    ...
    def authenticate_pre_sign_in(self, session_id, session_hash, user):
        ...
        resp = make_response(redirect(app.config['LOGOUT_URL'] + '?service=https://' + request.host_url + url_for('SessionView:authenticate_sign_in', session_id=session_id, session_hash=session_hash, user=user)))
