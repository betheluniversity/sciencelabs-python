...
class Session:
    ...
    def get_closed_sessions(self, semester_id):
        return (self.sess.query(Session_Table)
    ...
    def get_available_sessions(self, semester_id):
        return self.sess.query(Session_Table).filter(Session_Table.semester_id == semester_id)\
    ...
    def get_open_sessions(self):
        return self.sess.query(Session_Table).filter(Session_Table.open == 1).all()

    def get_session_tutors(self, session_id):
        return self.sess.query(User_Table.id, User_Table.firstName, User_Table.lastName, TutorSession_Table.isLead,
    ...
    def get_tutor_session_info(self, tutor_id, session_id):
        return self.sess.query(User_Table.firstName, User_Table.lastName, TutorSession_Table.isLead,
    ...
    def get_student_session_info(self, student_id, session_id):
        return self.sess.query(User_Table.id, User_Table.firstName, User_Table.lastName, StudentSession_Table.timeIn,
    ...
    def get_student_session_courses(self, session_id, student_id):
        return self.sess.query(Course_Table.id, Course_Table.dept, Course_Table.course_num, CourseCode_Table.courseName)\
            .filter(StudentSession_Table.sessionId == session_id)\
            .filter(StudentSession_Table.studentId == student_id)\
    ...
    def get_course_email_info(self, course_id):
        return self.sess.query(Course_Table.title, Course_Table.section)\
            .filter(Course_Table.id == course_id)\
            .all()


    def get_other_course(self, session_id, student_id):
        return self.sess.query(StudentSession_Table.otherCourse, StudentSession_Table.otherCourseName)\
            .filter(StudentSession_Table.sessionId == session_id)\
            .filter(StudentSession_Table.studentId == student_id)\
            .one()
    ...
    def get_session_attendees(self, session_id):
        return self.sess.query(User_Table.id) \
            .filter(StudentSession_Table.sessionId == session_id)\
            .filter(StudentSession_Table.studentId == User_Table.id)\
            .distinct()

    def get_session_attendees_with_dup(self, course_id, session_id):
        return self.sess.query(StudentSession_Table, func.count(distinct(StudentSession_Table.id)))\
            .filter(StudentSession_Table.sessionId == session_id)\
            .filter(Session_Table.id == StudentSession_Table.sessionId)\
    ...
    def add_anonymous_to_session(self, session_id, anon_students):
        session_to_edit = self.sess.query(Session_Table)\
            .filter(Session_Table.id == session_id)\
            .one()
    ...
    def add_student_to_session(self, session_id, student_id):
        ...
        self.sess.add(new_student_session)
        self.sess.commit()

    def delete_student_from_session(self, student_id, session_id):
        student_session_to_delete = self.sess.query(StudentSession_Table)\
            .filter(StudentSession_Table.studentId == student_id)\
            .filter(StudentSession_Table.sessionId == session_id)\
            .one()
        self.sess.delete(student_session_to_delete)
        self.sess.commit()

    def delete_tutor_from_session(self, tutor_id, session_id):
        tutor_session_to_delete = self.sess.query(TutorSession_Table)\
            .filter(TutorSession_Table.tutorId == tutor_id)\
            .filter(TutorSession_Table.sessionId == session_id)\
            .one()
        self.sess.delete(tutor_session_to_delete)
        self.sess.commit()

    def edit_tutor_session(self, session_id, tutor_id, time_in, time_out, lead):
        tutor_session_to_edit = self.sess.query(TutorSession_Table)\
            .filter(TutorSession_Table.sessionId == session_id)\
            .filter(TutorSession_Table.tutorId == tutor_id)\
            .one()
    ...
    def get_avg_total_time_per_student(self, semester_id):
        return self.sess.query(StudentSession_Table) \
            .filter(StudentSession_Table.sessionId == Session_Table.id) \
            .filter(Session_Table.semester_id == Semester_Table.id) \
            .filter(Semester_Table.id == semester_id) \
    ...
    def get_years(self):
        return self.sess.query(Semester_Table.year)\
    ...
    def tutor_currently_signed_in(self, session_id, tutor_id):
        return self.sess.query(TutorSession_Table).filter(TutorSession_Table.sessionId == session_id)\
    ...
    def student_currently_signed_in(self, session_id, student_id):
        return self.sess.query(StudentSession_Table).filter(StudentSession_Table.sessionId == session_id)\
    ...
    def edit_student_session_info(self, session_id, student_id, time_in, time_out, other_course):
        student_session_to_edit = self.sess.query(StudentSession_Table) \
            .filter(StudentSession_Table.sessionId == session_id) \
    ...
    def edit_student_courses(self, session_id, student_id, student_courses):
        student_session = self.sess.query(StudentSession_Table).filter(StudentSession_Table.studentId == student_id) \
            .filter(StudentSession_Table.sessionId == session_id).one()
        self.sess.query(SessionCourses_Table).filter(
        ...
        for course in student_courses:
            ...
            self.sess.add(new_student_course)
        self.sess.commit()
    ...
    def create_session_courses(self, session_id, courses):
        for course in courses:
            ...
            self.sess.add(new_session_course)
        self.sess.commit()
    ...
    def edit_session_info(self, session_id, semester_id, date, scheduled_start, scheduled_end, actual_start, actual_end,
        ...
        session_to_edit = self.sess.query(Session_Table).filter(Session_Table.id == session_id).one()
    ...
    def edit_session_leads(self, scheduled_start, scheduled_end, leads, session_id):
        current_lead_ids = self.sess.query(TutorSession_Table.tutorId).filter(TutorSession_Table.sessionId == session_id)\
        ...
        for current_lead in current_lead_ids:
            if current_lead in leads:
                ...
                self.sess.query(TutorSession_Table).filter(TutorSession_Table.sessionId == session_id)\
    ...
    def edit_session_tutors(self, scheduled_start, scheduled_end, tutors, session_id):
        current_tutor_ids = self.sess.query(TutorSession_Table.tutorId).filter(TutorSession_Table.sessionId == session_id) \
    ...
    def edit_session_courses(self, session_id, courses):
        self.sess.query(SessionCourseCodes_Table).filter(SessionCourseCodes_Table.session_id == session_id).delete()
        self.sess.commit()
    ...
    def restore_deleted_session(self, session_id):
        session_to_restore = self.sess.query(Session_Table).filter(Session_Table.id == session_id).one()
        session_to_restore.deletedAt = None
        self.sess.commit()

    def start_open_session(self, session_id, opener_id):
        session_to_open = self.sess.query(Session_Table).filter(Session_Table.id == session_id).one()
    ...
    def close_open_session(self, session_id, comments):
        session_to_close = self.sess.query(Session_Table).filter(Session_Table.id == session_id).one()
    ...
    def tutor_sign_in(self, session_id, tutor_id):
        tutor_session = self.sess.query(TutorSession_Table).filter(TutorSession_Table.sessionId == session_id)\
    ...
    def student_sign_out(self, session_id, student_id):
        student_session = self.sess.query(StudentSession_Table).filter(StudentSession_Table.sessionId == session_id)\
            .filter(StudentSession_Table.studentId == student_id).one()
