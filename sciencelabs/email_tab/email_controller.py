...
class EmailController:
            ...
            if 'Administrator' in recipient_role_names:
                ...
                for course in session_courses:
                    courses_and_info[course] = {
                        'students': self.session.get_session_course_students(session_id, course.id),  # Gets students for a course in a specific session
                        'profs': self.course.get_profs_from_course(course.id)
                    }
                ...
                for course in session_courses:
                    if course.id in professor_course_ids:
                        courses_and_info[course] = {
                            'students': self.session.get_session_course_students(session_id, course.id),  # Same as above
                            'profs': self.course.get_profs_from_course(course.id)
                        }
    ...
    def reservation_confirm_or_cancel(self, session_id, recipient, student_courses=None, deleted=False):
        sess = self.session.get_session(session_id)
        reservation = self.session.get_reservation(sess.id, recipient.id)
        courses_ids = self.session.get_reservation_courses(reservation.id)
        ...
        if deleted:
            subject = 'F2F Reservation Cancelled, {0} ({1})'.format(sess.name, sess.date.strftime('%m/%d/%Y'))
